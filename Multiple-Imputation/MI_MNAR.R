...
library(mice)
library(missForest)
setwd("C:/Users/user/Desktop/I/ewha2/GML2")
df <- read_csv("transfusion.csv")

# x1 "Recency (months)"                          
# x2 "Frequency (times)"                         
# x3 "Monetary (c.c. blood)"                     
# x4 "Time (months)"                             
# y "whether he/she donated blood in March 2007"
...
set.seed(1234)
z <- rnorm(n,-0.55,1)
U1 <- a1*scale(x3) + a2*scale(y) + z
df$x3_na <- ifelse(U1>0,NA,x3)

set.seed(0804)
z <- rnorm(n,-0.55,1)
U2 <- a1*scale(x4) + a2*scale(y) + z
...
write.csv(train,'train_MNAR.csv')
write.csv(test,'test_MNAR.csv')
...
rmse <- function(train,test){
  ...
  sqrt(sum((train$x3-train$x3_imput)^2)/93) + 
    sqrt(sum((train$x4-train$x4_imput)^2)/101) +
    sqrt(sum((test$x3-test$x3_imput)^2)/36) + 
    sqrt(sum((test$x4-test$x4_imput)^2)/49)
}

UCE <- function(train,test){
  myglm <- glm(y ~ x1+x2+x3_imput+x4_imput, data = train)
...
# 2) train + test
df_new <- as.data.frame(rbind(train2[,c(1,2,6,7)],test2[,c(1,2,6,7)]))
...
train_mice <- cbind(df_mice[1:315,],train2[,c(3,4,5)])
test_mice <- cbind(df_mice[316:449,],test2[,c(3,4,5)])
rmse(train_mice,test_mice)
UCE(train_mice,test_mice)
