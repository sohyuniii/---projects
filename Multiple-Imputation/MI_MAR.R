...
write.csv(train,'train_MAR.csv')
write.csv(test,'test_MAR.csv')
...
rmse <- function(train,test){
  ...
  sqrt(sum((train$x3-train$x3_imput)^2)/92) + 
    sqrt(sum((train$x4-train$x4_imput)^2)/106) +
    sqrt(sum((test$x3-test$x3_imput)^2)/40) + 
    sqrt(sum((test$x4-test$x4_imput)^2)/43)
}
...
test_knn <- cbind(df_knn[316:n,],test2[,c(3,4,5)])
rmse(train_knn,test_knn)
UCE(train_knn,test_knn)
...
test_miss <- cbind(df_miss[316:n,],test4[,c(3,4,5)])
rmse(train_miss,test_miss)
UCE(train_miss,test_miss)
